...
module.exports.handleEvent = async function({ api, event, args, Threads, Users }) {
  ...
  var tl = ["تحتاج شيئا عزيزي ؟ " , "اتركني لست في مزاج جيد " , "هل تريد ان تعترف لي بشيء 🤭" , "اشتقت لك 🥰" , "انا في خدمتك" , "لن اجيبك لان مجد يغار 🤭❤️" , "هففف ماذا مجددا " , "لوسيا لوسيا لوسيا ارحموني🤬" , "لوسي الحلوة فخدمتك" , " انا تحت امرك يا سيد" , "لن تتعبو من مناداتي ؟ 😠👊🏻"];  var rand = tl[Math.floor(Math.random() * tl.length)]
   ...
   if ((event.body.toLowerCase() == "بوت") || (event.body.toLowerCase() == "يا بوت")) {
     return api.sendMessage(" نادني لوسيا او لوسي 😑", threadID);
   };
   ...
   if ((event.body.toLowerCase() == "احمد ") || (event.body.toLowerCase() == "كايت") || (event.body.toLowerCase() == "kite") || (event.body.toLowerCase() == "majd")) {
     return api.sendMessage( "عزيزي احمد مشغول الان  😘",threadID);

       
   };

   if ((event.body.toLowerCase() == "المالك") || (event.body.toLowerCase() == "المطور")) {
     return api.sendMessage("‎[المالك:☞SMILE ☜ تستطيع مناداته احمد.\n :\n-\nتواصل معه على الواتساب :\n- +966535252928", threadID);
   };

   if ((event.body.toLowerCase() == "@M. Elhalimi") || (event.body.toLowerCase() == "احمد")) {
  ...
  if ((event.body.toLowerCase() == "ادمن") || (event.body.toLowerCase() == "صند ادمن")) {
   ...
   if ((event.body.toLowerCase() == "جيد") || (event.body.toLowerCase() == "شكرا") || (event.body.toLowerCase() == "شكرا لك") || (event.body.toLowerCase() == "شكؤا لك لوسيا")) {
   ...
   if ((event.body.toLowerCase() == "ما اسمك") || (event.body.toLowerCase() == "اسمك") || (event.body.toLowerCase() == "ما اسمها")) {
     return api.sendMessage("️لوسي تشرفت بمعرفتك.", threadID);
   };
  ...
  if (event.body.indexOf("لوسيا") == 0 || (event.body.indexOf("لوسي") == 0)) {
    var msg = {
      body: `${rand}`
    }
