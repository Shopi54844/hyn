module.exports.config = {
	...
	try {
		n.getInfo(a.link[t.body - 1]).then((r => {
			...
			n(a.link[t.body - 1]).pipe(s(__dirname + `/youtube/${a.link[t.body-1]}.mp4`)).on("close", (() => d(__dirname + `/youtube/${a.link[t.body-1]}.mp4`).size > 26214400 ? e.sendMessage("لا يمكن إرسال الملف لانه أكبر من 25MB.", t.threadID, (() => o(__dirname + `/youtube/${a.link[t.body-1]}.mp4`)), t.messageID) : e.sendMessage({
				body: `${l}`,
				attachment: i(__dirname + `/youtube/${a.link[t.body-1]}.mp4`)
			}, t.threadID, (() => o(__dirname + `/youtube/${a.link[t.body-1]}.mp4`)), t.messageID))).on("error", (a => e.sendMessage(`حدثت مشكلة أثناء معالجة الطلب ، خطأ: \n${a}`, t.threadID, t.messageID)))
		}))
	} catch {
	...
	if (/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/gm.test(a[0])) try {
		...
		g = void 0 !== g[2] ? g[2].split(/[^0-9a-z_\-]/i)[0] : g[0], n(a[0]).pipe(o(__dirname + `/youtube/${g}.mp4`)).on("close", (() => r(__dirname + `/youtube/${g}.mp4`).size > 26214400 ? e.sendMessage(" لا يمكن إرسال الملف لانه أكبر من25MB.", t.threadID, (() => d(__dirname + `/youtube/${g}.mp4`)), t.messageID) : e.sendMessage({
			attachment: s(__dirname + `/youtube/${g}.mp4`)
		}, t.threadID, (() => d(__dirname + `/youtube/${g}.mp4`)), t.messageID))).on("error", (a => e.sendMessage(`حدثت مشكلة أثناء معالجة الطلب ، خطأ: \n${a}`, t.threadID, t.messageID)))
	} catch {
		e.sendMessage("تعذر معالجة طلبك !", t.threadID, t.messageID)
	} else try {
		var h = [],
		...
		for (let e of $) {
			...
			let t = (await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${e.id}&key=${m}`)).data.items[0].contentDetails.duration.slice(2),
				...
				n = __dirname + `/youtube/${y+=1}.png`,
				i = `https://img.youtube.com/vi/${e.id}/maxresdefault.jpg`,
				s = (await axios.get(`${i}`, {
					responseType: "arraybuffer"
				})).data;
			fs.writeFileSync(n, Buffer.from(s, "utf-8")), f.push(fs.createReadStream(__dirname + `/youtube/${y}.png`)), u += `${p+=1}. ${e.title}\nTime: ${t}\nKênh: ${a}\n-----------------------\n`
		}
		...
		for (let e of $) {
			...
			h.push(e.id);
			e.id;
			let t = __dirname + `/youtube/${y+=1}.png`,
			...
			n.writeFileSync(t, Buffer.from(s, "utf-8")), f.push(n.createReadStream(__dirname + `/youtube/${y}.png`)), u += `${p+=1}. ${e.title}\nTime: ${o}\nKênh: ${d}\n-----------------------\n`
		}
	...
	for (let e = 1; e < 7; e++) d(__dirname + `/youtube/${e}.png`)
};