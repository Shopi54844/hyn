...
if(!isNaN(args[0])) {
    ...
    let bg = (await axios.get(`https://i.postimg.cc/nr6fztTW/Picsart-22-07-28-16-29-29-414.jpg`, {responseType: "arraybuffer" })).data;
    ...
    ctx.fillText('-' + args[0].replace(/\B(?=(\d{3})+(?!\d))/g, '.') + '$', 290, 270);
    ctx.font = "25px SplineSans";
    ctx.fillText(`${content}`, 85, 110);
    ctx.font = "26px SplineSans-Medium";
    ctx.textAlign = "right";
    ctx.fillText(`${userName}`, 335, 515);
