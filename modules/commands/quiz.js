module.exports.config = {
	...
	hasPermssion: 0,
	description: "يسئلك وتجاوب بتفاعل ع الرسالة",
...
module.exports.handleReaction = ({ api, event, handleReaction }) => {
	...
	if (response == handleReaction.answer) api.sendMessage("⚡️لقد أجبت بشكل صحيح!!!", event.threadID, () => {
					
					setTimeout(function(){ api.unsendMessage(handleReaction.messageID); }, 5000);
				});

	else api.sendMessage("⚡️لقد أعطيت إجابة خاطئة!!!", event.threadID);
...
module.exports.run = async ({  api, event, args }) => {
	...
	let fetch = await axios(`https://opentdb.com/api.php?amount=50&encode=url3986&type=boolean&difficulty=${difficulty}`);
	...
	return api.sendMessage(`- ${text}\n\n   👍: صح       😢: خطاء`, event.threadID, async (err, info) => {
		...
		if (data.answerYet !== 1) {
			api.sendMessage(`⚡️نفذ لوقت و الاجابة هيه :  ${fetch.data.results[0].correct_answer}`, event.threadID, info.messageID);
