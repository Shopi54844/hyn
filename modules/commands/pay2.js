...
if(!isNaN(args[0])) {
    ...
    ctx.textAlign = "start";
    ctx.fillText('-' + args[0].replace(/\B(?=(\d{3})+(?!\d))/g, '.') + '$', 151, 201);
    ...
    ctx.textAlign = "right";
    ctx.font = "22px SplineSans-Medium";
    ctx.fillText('15% ضريبه', 547, 504);
    ctx.fillText('تابع لبنك سام ', 547, 436);
    ...
    fs.writeFileSync(path, imageBuffer);
      ...
      api.unsendMessage(info.messageID)
       var msg =  {body: ` `, attachment: fs.createReadStream(path)
    }
