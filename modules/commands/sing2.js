module.exports.config = {
	...
	description: "YouTube تنزيل الاغاني من ",
...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	api.sendMessage(`🎵[ جار تنزيل الاغنية ] !\n-----------\n${body}\n-----------\n  انتضر !`, event.threadID, (err, info) =>
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`).size > 26214400) return api.' تعذر إرسال الملف لأنه أكبر من 25 ميغا بايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`), event.messageID);
...
module.exports.run = async function({ api, event, args }) {
	...
	if (args.length == 0 || !args) return api.sendMessage('📢 لا يمكن ترك البحث فارغًا !', event.threadID, event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.m4a`).size > 26214400) return api.sendMessage('❗تعذر إرسال الملف لأنه أكبر من 25 ميغا بايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.m4a`), event.messageID);
