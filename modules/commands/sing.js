module.exports.config = {
	...
	hasPermssion: 0,
	credits: "Horizon",
	description: "YouTube و SoundCloud تنزيل الاغاني من ",
...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {   
		await ytdl(handleReply.link[event.body - 1],{ filter: 'audioonly'})
			.pipe(createWriteStream(__dirname + `/youtube/${handleReply.link[event.body - 1]}.m4a`))
			.on("close", () => {
				if (statSync(__dirname + `/youtube/${handleReply.link[event.body - 1]}.m4a`).size > 26000000) return api.sendMessage('⚡𝑩𝒂̀𝒊 𝒏𝒂̀𝒚 𝒒𝒖𝒂́ 𝒅𝒂̀𝒊 𝒃𝒐𝒕 𝒌𝒉𝒐̂𝒏𝒈 𝒕𝒉𝒆̂̉ 𝒈𝒖̛̉𝒊 đ𝒖̛𝒐̛̣𝒄, Đ𝒐̂̉𝒊 𝒃𝒂̀𝒊 đ𝒊 𝒏𝒉𝒆́!', event.threadID, () => unlinkSync(__dirname + `/youtube/${handleReply.link[event.body - 1]}.m4a`), event.messageID);
				else return api.sendMessage({body: `جار التحميل :\n${title} `,attachment: createReadStream(__dirname + `/youtube/${handleReply.link[event.body - 1]}.m4a`)}, event.threadID, event.messageID);
			})
			.on("error", (error) => api.sendMessage(`خطأ : \n${error}`, event.threadID, event.messageID));
		}
	catch (e) {
		console.log(e)
...
module.exports.run = async function({ api, event, args,help }) {
	...
	if (urlValid) {
		try { 
			...
			ytdl(args[0])
				.pipe(createWriteStream(__dirname + `/youtube/${id}.m4a`))
				.on("close", () => {
					if (statSync(__dirname + `/youtube/${id}.m4a`).size > 26214400) return api.sendMessage('⚡𝑩𝒂̀𝒊 𝒏𝒂̀𝒚 𝒒𝒖𝒂́ 𝒅𝒂̀𝒊 𝒃𝒐𝒕 𝒌𝒉𝒐̂𝒏𝒈 𝒕𝒉𝒆̂̉ 𝒈𝒖̛̉𝒊 đ𝒖̛𝒐̛̣𝒄, Đ𝒐̂̉𝒊 𝒃𝒂̀𝒊 đ𝒊 𝒏𝒉𝒆 🥺', event.threadID, () => unlinkSync(__dirname + `/youtube/${id}.m4a`), event.messageID);
					else{
						 api.sendMessage({attachment: createReadStream(__dirname + `/youtube/${id}.m4a`)}, event.threadID, event.messageID)
 ...
 \n━━━━━━━ •♬• ━━━━━━━\n${msg}📌 قم بالرد على هذه الرسالة  برقم الأغنية للتحميل `, event.threadID,(error, info) => global.client.handleReply.push({ name: this.config.name, messageID: info.messageID, author: event.senderID, link: link }), event.messageID);
		}
		catch (error) {
