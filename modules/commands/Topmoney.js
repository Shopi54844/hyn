...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
  ...
  return Math.floor((Math.sqrt(1 + (4 * point) / 3) + 1) / 2);
    ...
    let all = await Currencies.getAll(['userID', 'exp']);
        ...
        let num = 0;
               let msg = {
          body: '⚡️ اعلى 15 مستخدم على سيرفر البوت !',
  ...
  if (args[0] == "thread") {
    ...
    for(const dataThread of threadList) {
      if (i == option) break;
      msg += `⚡${i+1}/ ${(dataThread.threadName)||"بدون اسم"}\n⚡TID: [${dataThread.threadID}]\n⚡عدد الرسائل: ${dataThread.messageCount} رسالة \n\n`;
      i+=1;
    }
    
    return api.sendMessage(`⚡️ها هو الجزء العلوي ${threadList.length} اكثر المجموعات تفاعلا البوت البوت :\n≻───── ⋆✩⋆ ─────≺\n${msg}\n≻────END────≺`, threadID, messageID);
  }
 ...
 if (args[0] == "money") { 
    let all = await Currencies.getAll(['userID', 'money']);
        ...
        let num = 0;
               let msg = {
          body: '⚡️اغنى 15 شخص على سيرفر البوت  !',
